function l(a){const s=(Math.round(a*100)/100).toFixed(2).split(".");return s[0].replace(/\B(?=(\d{3})+(?!\d))/g," ")+","+s[1]}function y(a){if(!a||a.length<10)return a||"";const i=a.split("-");return i.length!==3?a:i[2]+"."+i[1]+"."+i[0]}function e(a){return a?a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}function P(a){const{invoiceNumber:i,sequenceNumber:c,issuedDate:s,taxPointDate:n,dueDate:g,variableSymbol:f,issuerName:m,issuerAddress:d,issuerIC:r,issuerDIC:x,issuerRegistration:w,issuerPhone:$,issuerEmail:z,issuedBy:k,bankAccount:R,bankIban:q,customerName:B,customerAddress:E,customerIC:H,customerDIC:D,lineItems:F,qrDataUrl:T,invoiceNote:N}=a,v=new Map;let b=0,u=0;(F||[]).forEach(t=>{const o=t.unitPriceCzk*t.quantity,p=Math.round(o*(t.dphRate/100)*100)/100;b+=o,u+=p;const h=v.get(t.dphRate)||{base:0,tax:0,total:0};h.base+=o,h.tax+=p,h.total+=o+p,v.set(t.dphRate,h)});const I=Math.round((b+u)*100)/100,O=e(N||`Rezervace vašeho prodejního místa se stává závaznou až po uhrazení této faktury. V případě, že nebude faktura uhrazena do data splatnosti, rezervace automaticky zaniká a místo bude nabídnuto dalšímu vystavovateli.
Při zrušení účasti méně než 14 dní před termínem akce činí storno poplatek 100 % z celkové částky.`).replace(/\n/g,"<br>"),M=(d||"").split(`
`).filter(Boolean),S=(E||"").split(`
`).filter(Boolean);let A="";for(const[t,o]of v.entries())A+=`<tr>
            <td>${t===21?"Základní":t+"%"}</td>
            <td>${t} %</td>
            <td class="r">${l(o.base)}</td>
            <td class="r">${l(o.tax)}</td>
            <td class="r">${l(o.total)}</td>
        </tr>`;let C="";return(F||[]).forEach((t,o)=>{const p=t.unitPriceCzk*t.quantity;C+=`<tr>
            <td>${o+1}</td>
            <td class="desc">${e(t.description)}</td>
            <td>${t.quantity}</td>
            <td>ks</td>
            <td>${t.dphRate}</td>
            <td class="r">${l(t.unitPriceCzk)}</td>
            <td class="r">${l(p)}</td>
        </tr>`}),`<!DOCTYPE html>
<html lang="cs">
<head>
...
<div class="page">
    ...
    <div class="meta">
        <div class="meta-col">
            <div class="meta-row"><span class="meta-label">Datum vystavení:</span><span class="meta-val">${y(s)}</span></div>
            <div class="meta-row"><span class="meta-label">Datum úč.zd.plnění:</span><span class="meta-val">${y(n)}</span></div>
            <div class="meta-row"><span class="meta-label">Datum splatnosti:</span><span class="meta-val bold">${y(g)}</span></div>
        </div>
        <div class="meta-col">
            <div class="meta-row"><span class="meta-label">Způsob úhrady:</span><span class="meta-val">Převodním příkazem</span></div>
            <div class="meta-row"><span class="meta-label">Bankovní spojení:</span><span class="meta-val">${e(R)}</span></div>
            <div class="meta-row"><span class="meta-label">IBAN:</span><span class="meta-val sm">${e(q)}</span></div>
    ...
    <div class="parties">
        <div class="party">
            ...
            ${M.map(t=>`<div class="party-line">${e(t)}</div>`).join("")}
            <div class="party-ic">IČ: ${e(r)}${x?"  DIČ: "+e(x):""}</div>
        ...
        <div class="party">
            ...
            <div class="party-name">${e(B)}</div>
            ${S.map(t=>`<div class="party-line">${e(t)}</div>`).join("")}
            <div class="party-ic">IČ: ${e(H)}${D?"  DIČ: "+e(D):""}</div>
        </div>
    </div>

    <!-- Note -->
    <div class="note">${O}</div>
    ...
    <div class="grand-total">
        Celkem k úhradě: <span>${l(I)} CZK</span>
    </div>
    ...
    <div class="footer">
        <div>
            ...
            ${z?`<div class="footer-text">${e(z)}</div>`:""}
        </div>
        <div class="footer-isdoc">ISDOC 6.0.1</div>
        <div style="text-align:right">
            ${k?`<div class="footer-text">Vystavil: ${e(k)}</div>`:""}
...
</html>`}function V(a,i){var m;const c=P(a),s=document.createElement("iframe");s.style.cssText="position:fixed;right:0;bottom:0;width:0;height:0;border:none;opacity:0",document.body.appendChild(s);const n=s.contentDocument||((m=s.contentWindow)==null?void 0:m.document);if(!n){document.body.removeChild(s);return}n.open(),n.write(c),n.close();const g=n.querySelectorAll("img"),f=Array.from(g).map(d=>new Promise(r=>d.complete?r():(d.onload=()=>r(),d.onerror=()=>r())));Promise.all(f).then(()=>{setTimeout(()=>{var d;try{(d=s.contentWindow)==null||d.print()}catch(r){console.error("[Invoice] Print failed:",r)}setTimeout(()=>document.body.removeChild(s),1e3)},200)})}async function Z(a){return new Blob([P(a)],{type:"text/html"})}export{V as downloadInvoiceAsPdf,Z as generateInvoiceBlobFromHtml,P as generateInvoiceHtml};
