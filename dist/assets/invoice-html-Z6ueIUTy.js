function o(a){const n=(Math.round(a*100)/100).toFixed(2).split(".");return n[0].replace(/\B(?=(\d{3})+(?!\d))/g," ")+","+n[1]}function m(a){if(!a||a.length<10)return a||"";const i=a.split("-");return i.length!==3?a:i[2]+"."+i[1]+"."+i[0]}function e(a){return a?a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}function V(a){const{invoiceNumber:i,sequenceNumber:g,issuedDate:n,taxPointDate:h,dueDate:F,variableSymbol:T,issuerName:I,issuerAddress:C,issuerIC:P,issuerDIC:v,issuerRegistration:f,issuerPhone:b,issuerEmail:x,issuedBy:u,bankAccount:A,bankIban:B,customerName:R,customerAddress:E,customerIC:j,customerDIC:y,lineItems:w,qrDataUrl:$,invoiceNote:q}=a,r=new Map;let p=0,c=0;(w||[]).forEach(t=>{const s=t.unitPriceCzk*t.quantity,l=Math.round(s*(t.dphRate/100)*100)/100;p+=s,c+=l;const d=r.get(t.dphRate)||{base:0,tax:0,total:0};d.base+=s,d.tax+=l,d.total+=s+l,r.set(t.dphRate,d)});const k=Math.round((p+c)*100)/100,H=e(q||`Rezervace vašeho prodejního místa se stává závaznou až po uhrazení této objednávky. V případě, že nebude objednávka uhrazena do data splatnosti, rezervace automaticky zaniká a místo bude nabídnuto dalšímu vystavovateli.
Při zrušení účasti méně než 14 dní před termínem akce činí storno poplatek 100 % z celkové částky.`).replace(/\n/g,"<br>"),N=(C||"").split(`
`).filter(Boolean),M=(E||"").split(`
`).filter(Boolean);let z="";for(const[t,s]of r.entries())z+=`<tr>
            <td>${t===21?"Základní":t+"%"}</td>
            <td>${t} %</td>
            <td class="r">${o(s.base)}</td>
            <td class="r">${o(s.tax)}</td>
            <td class="r">${o(s.total)}</td>
            ...
            <td>ks</td>
            <td>${t.dphRate}</td>
            <td class="r">${o(t.unitPriceCzk)}</td>
            <td class="r">${o(l)}</td>
        </tr>`}),`<!DOCTYPE html>
<html lang="cs">
<head>
<meta charset="UTF-8">
<title>Objednávka ${e(i)}</title>
<style>
...
<div class="page">
    ...
    <div class="meta">
        ...
        <div class="meta-col">
            ...
            <div class="meta-row"><span class="meta-label">Bankovní spojení:</span><span class="meta-val">${e(A)}</span></div>
    ...
    <div class="parties">
        <div class="party">
            ...
            <div class="party-ic">IČ: ${e(P)}${v?"  DIČ: "+e(v):""}</div>
            ${f?`<div class="party-line" style="font-size:7pt;color:#888;margin-top:2pt">${e(f)}</div>`:""}
        </div>
        <div class="party">
            <span class="party-seq">#${g}</span>
            <div class="party-label">Odběratel</div>
            <div class="party-name">${e(R)}</div>
            ${M.map(t=>`<div class="party-line">${e(t)}</div>`).join("")}
            <div class="party-ic">IČ: ${e(j)}${y?"  DIČ: "+e(y):""}</div>
        </div>
    </div>
    ...
    <div class="grand-total">
        Celkem k úhradě: <span>${o(k)} CZK</span>
    </div>
...
</html>`}export{V as generateInvoiceHtml};
