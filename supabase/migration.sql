...
CREATE TABLE IF NOT EXISTS applications (
  ...
  zone_category TEXT,
  status TEXT NOT NULL DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'APPROVED', 'APPROVED_FREE', 'REJECTED', 'WAITLIST', 'PAID', 'EXPIRED', 'PAYMENT_REMINDER', 'PAYMENT_LAST_CALL', 'PAYMENT_UNDER_REVIEW', 'DELETED')),
...
Tým LAVRS market', 'application', true),
  ...
  ('application-waitlist', 'Schválení přihlášky, není kapacita, možnost waitlistu', 'Výsledek přihlášky na {{event_name}}', 'Schválení přihlášky do budoucna (nyní není místo).', 'Dobrý den, {{contact_person}},
...
-- Remove deprecated event templates
DELETE FROM email_templates WHERE id IN ('event-instructions', 'event-reminder', 'post-event');
...
-- ═══════════════════════════════════════════════════════════
-- 18. Admin notification template (when payment is confirmed)
-- ═══════════════════════════════════════════════════════════
INSERT INTO email_templates (id, name, subject, description, body, category, enabled) VALUES
  ('payment-approved-admin', 'Platba ověřena (admin notifikace)', 'Platba ověřena: {{brand_name}} — {{event_name}}', 'Automatický email odeslaný adminovi když je platba ověřena (status PAID).', 'Platba byla ověřena!

{{order_table}}

Vystavovatel {{brand_name}} zaplatil účast na {{event_name}} ({{event_date}}).

Číslo faktury: {{invoice_number}}
Částka: {{invoice_amount}} Kč

Místo je nyní závazně rezervováno.', 'payment', true)
ON CONFLICT (id) DO NOTHING;

-- ═══════════════════════════════════════════════════════════
-- 19. DATABASE TRIGGERS PRO EMAIL WEBHOOK (2026-04-14)
-- ═══════════════════════════════════════════════════════════
-- Extension pro HTTP callouts (pokud již není aktivní)
CREATE EXTENSION IF NOT EXISTS pg_net;

-- Funkce pro spuštění webhoooku na edge function
...
-- Trigger na INSERT - když se vytvoří nová aplikace
...
-- Trigger na UPDATE - když se změní status (a jen když se status změní)
